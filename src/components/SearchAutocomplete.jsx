...
export default function SearchAutocomplete({ onNavigate, mobile = false }) {
  ...
  return (
    <div ref={wrapperRef} className={`relative ${mobile ? 'w-full' : 'flex-1 max-w-md'}`}>
      ...
      {open && (
        <div className="absolute top-full left-0 right-0 mt-1 bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl overflow-hidden z-[100] animate-fade-in-up">
          {results.length === 0 ? (
            <div className="px-4 py-4">
              <p className="text-sm font-medium text-zinc-200">No encontramos resultados para "{query}".</p>
              <p className="text-xs text-zinc-400 mt-1">Probá con otra marca, nombre o SKU.</p>
            </div>
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-colors ${
                index === selectedIndex ? 'bg-red-950/40' : 'hover:bg-zinc-950'
            ...
            >
              ...
              <span className="text-sm font-bold text-red-400 shrink-0">{product.showPrice === false ? 'Consultar' : formatPrice(product.price)}</span>
            </button>
          ))}
          <button
            onClick={goToSearch}
            className="w-full flex items-center justify-center gap-2 px-4 py-3 text-sm font-semibold text-red-400 hover:bg-zinc-800 border-t border-zinc-800 transition-colors"
          >
